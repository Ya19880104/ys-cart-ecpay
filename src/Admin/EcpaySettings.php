...
final class EcpaySettings {
	...
	private static function desired_credentials_group( string $prefix, array $keys ): array {
		if ( isset( $_POST[ 'ys_ec_ecpay_' . $prefix . '_clear' ] ) ) {
			...
			if ( isset( $keys['credit_check_code'] ) ) {
				// v0.3.0：清空 payment 憑證時一併清查詢檢查碼——留下孤兒檢查碼
				// 會讓下一組憑證誤用上一個商店的 CheckCode 打查詢端點。
				$cleared[ $keys['credit_check_code'] ] = '';
			}
			return $cleared;
		}
	...
	public static function settings_for_render(): array {
		...
		foreach ( [
			...
			if ( isset( $keys['credit_check_code'] ) ) {
				// v0.3.0：商家檢查碼 CreditCheckCode（僅 payment 群組有；綠界官方名稱，選填）
