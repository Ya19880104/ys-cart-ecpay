...
namespace {
    ...
    $assert(
		str_contains( $bootstrap_src, "define( 'YS_CART_ECPAY_REQUIRES_CORE', '2.67.11' );" ),
		'(h) 宣告的最低核心版本是 2.67.11（attempt-safe callback 與 payable handoff 契約）'
    );
    ...
    // 核心太舊（2.56.12 < 2.67.11）→ met=false／core_too_old，訊息帶出所需版本。
    define( 'YS_ECOMMERCE_VERSION', '2.56.12' );
    $gate_old = Plugin::core_requirements();
    $assert(
        false === $gate_old['met']
        && 'core_too_old' === $gate_old['reason']
		&& str_contains( $gate_old['message'], '2.67.11' ),
        '(h4) 核心太舊 → core_too_old 且訊息帶出所需版本'
    );

    // 版本足夠時的放行路徑：define 不可重定義，無法在同一 process 內模擬 2.67.11；
