...
// v0.2.12：註冊迴圈由型錄驅動，因此不再逐一比對「id → new ClassName」的原始碼形狀
// （那會把舊實作鎖成契約）。改為驗兩件真正重要的事：
//   1. 每個方式的註冊都必須先過 is_method_enabled( 'shipping', <該方式的 id> )；
//   2. 型錄與類別檔案雙向對得起來——型錄列的類別存在，且該類別回報的 id 就是型錄的 key。
if (!defined('ABSPATH')) {
    define('ABSPATH', __DIR__ . '/');
}
require_once $root . '/src/Shipping/Ecpay/EcpayShippingCatalog.php';

$catalog = \YangSheep\YSCartEcpay\Shipping\Ecpay\EcpayShippingCatalog::all();

v009_check(
    'Shipping registration loop gates every catalog method on its own lifecycle switch',
    (bool) preg_match(
        '/foreach\s*\(\s*EcpayShippingCatalog::all\(\)\s+as\s+\$method_id\s*=>\s*\$descriptor\s*\)\s*\{\s*if\s*\(\s*!\s*\$this->is_method_enabled\(\s*\'shipping\'\s*,\s*\$method_id\s*\)\s*\)\s*\{\s*continue;/s',
        $plugin
    )
);

$catalog_bijective = [] !== $catalog;
foreach ($catalog as $method_id => $descriptor) {
    $file = $root . '/src/Shipping/Ecpay/' . substr((string) $descriptor['class'], strrpos((string) $descriptor['class'], '\\') + 1) . '.php';
    if (!is_file($file)) {
        $catalog_bijective = false;
        break;
    }
    $source = (string) file_get_contents($file);
    if (false === strpos($source, "return '{$method_id}';")) {
        $catalog_bijective = false;
        break;
    }
}
...
v009_check(
    'Every catalog entry has a class file whose get_id() returns the catalog key',
    $catalog_bijective
);
...
v009_check(
    ...
    false !== strpos($store, 'EcpayShippingCatalog::map_subtypes()')
        ...
        && false !== strpos($store, 'Settings::shipping_enabled( $alias )')
);
