...
> **2026-09-09 發版裁定（使用者授權）**：ys-cart-ecpay 0.3.2 已發布。上列 gate 的
> G-1～G-5／G-7／G-8／G-9 **仍然全部未通過**，本段不視為通過任何一項。發版範圍限於
> 外掛本體；**自動退款維持預設關閉**（`supports_gateway_refund()` 讀
> `ys_ec_ecpay_auto_refund_enabled`，預設 `0`），外掛未因發版而對外宣稱支援退款，
> `@deferred-live-verification` 標記全數保留。要開啟自動退款或對外宣稱支援退款，
> 仍須先完成上列 gate。
