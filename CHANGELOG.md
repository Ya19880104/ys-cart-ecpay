...
- AIO 與 ECPG callback 將 incoming MerchantTradeNo、attempt/dispatch、merchant、環境、金額與 scalar gateway 在同一 lifecycle CAS 重驗；每次 CAS 重試都以 fresh SQL row 對拍 `gateway_id` 與 `payment_method`，成功證據、TradeNo 與狀態不再分段寫入。只有 scalar gateway 為 NULL 的真正 pre-attempt legacy 列保留有界相容。
...
- B2C 與 C2C 物流仍可各自停用、共用金流設定或使用獨立設定；只有獨立設定會展開憑證欄位，已儲存金鑰仍維持留空不變更。
- 供應商啟用後才顯示對應設定，尚未支援的交易模式說明預設收合；付款、回呼、憑證來源與 lifecycle 語意不變。

## 0.5.8 - 2026-09-13

- 內嵌 YS Plugin Hub Client 同步至受審的 `2.0.7` full runtime，後台市集、外掛操作、設定與日誌傳輸改走 WordPress REST，並保留原有權限、資料 owner 與 first-loader 契約。付款、設定保存、資料移轉、ECPG 訂閱與物流行為不變。
